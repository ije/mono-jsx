  ...
  assertEquals(
    ...
    [
      ...
      ...Array.from({ length: 100 }).map((_, i) =>
        `<li class="black"><m-signal mode="[class]" scope="${i + 1}" computed="0" hidden></m-signal></li>`
      ),
      `</ul>`,
      `</body></html>`,
      `<script data-mono-jsx="${VERSION}">`,
...
Deno.test("[ssr] onFetch: not invoked without request", async () => {
  let calls = 0;
  const res = await Promise.resolve(
    <html lang="en" headers={{ setCookie: "foo=bar" }} onFetch={() => void calls++}>
      <body>
        <p>visible</p>
      </body>
    </html>,
  ...
  assert((await res.text()).includes("<p>visible</p>"));
});

Deno.test("[ssr] onFetch: request has URL and context is passed", async () => {
  const ctx = { id: 42 };
  ...
  assert(seenUrl instanceof URL);
  assertEquals(seenUrl.href, "https://example.com/foo?bar=1");
  assertEquals(seenContext, ctx);
});

Deno.test("[ssr] onFetch: returned Response short-circuits render", async () => {
  const res = await Promise.resolve(
    <html
      lang="en"
      request={new Request("https://example.com/")}
      onFetch={() => new Response("intercepted", { status: 202, headers: { "content-type": "text/plain" } })}
    >
      <body>
        <p>not rendered</p>
      </body>
...
Deno.test("[ssr] onFetch: void continues normal render", async () => {
  assertEquals(
    await renderToString(<p>through</p>, {
      request: new Request("https://example.com/"),
      onFetch: () => {},
    }),
    [
...
Deno.test("[ssr] onFetch: async Promise<Response>", async () => {
  const res = await Promise.resolve(
    <html
      lang="en"
      request={new Request("https://example.com/")}
      onFetch={async () => {
        await Promise.resolve();
        return new Response("async-body", { headers: { "content-type": "text/plain" } });
      }}
    >
      <body>x</body>
    </html>,
  );
  assertEquals(await res.text(), "async-body");
});
...
Deno.test("[ssr] <redirect>", async () => {
  ...
  assertEquals(
    ...
    [
      `<!DOCTYPE html>`,
      `<html lang="en"><body>`,
      `<m-redirect to="/dash" replace></m-redirect>`,
      `</body></html>`,
      `<script data-mono-jsx="${VERSION}">`,
      REDIRECT_JS,
      `</script>`,
  ...
  assertEquals(
    ...
    [
      `<!DOCTYPE html>`,
      `<html lang="en"><body>`,
      `<m-redirect to="https://example.com/dash"></m-redirect>`,
      `</body></html>`,
      `<script data-mono-jsx="${VERSION}">`,
      REDIRECT_JS,
      ,
